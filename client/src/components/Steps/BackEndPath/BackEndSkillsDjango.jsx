...
const BackEndSkillsDjango = ({ formData, setFormData }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [djangoFramework, setDjangoFramework] = useState(0);
  ...
  return (
    <div className="container-roadmap flex flex-col items-center justify-center min-h-screen bg-gray-100 px-4">
      <h2 className="text-2xl font-bold mb-6">
        Please rate your skills in the following areas of Django:
      </h2>
      <form
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2">
            ...
            <input
              ...
              max="10"
              className="w-full h-2 bg-gray-200 rounded outline-none slider-thumb"
              {...register("djangoFramework")}
              onChange={(e) => setDjangoFramework(e.target.value)}
              value={djangoFramework}
            />
          </label>
        </div>
        <div className="mb-4">
...
export default BackEndSkillsDjango;
