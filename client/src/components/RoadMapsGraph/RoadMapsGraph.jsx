...
export default function RoadMapsGraph() {
  ...
  async function fetchSpecifcRoadmap() {
    try {
      const { data } = await axios.get(
        `http://localhost:8000/api/v1/static-roadmaps/roadmap/${roadmapsgraph}`,
        {
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>career insight | {specificRoadmapData.name}</title>
      </Helmet>
      <div className="container-roadmap flex flex-col items-center justify-center min-h-screen bg-gray-100">
              ...
              {stages.map((stage, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {stageData.projects && stageData.projects.length > 0 && (
                      <>
                        {stageData.projects.map((project, index) => (
                          <div key={index}>
          ...
          <div className="col-md-4 flex flex-col justify-between items-center my-5 h-[280vh] md:h-[200vh]">
            <div className="p-4">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-3xl font-bold text-pc ml-2">
                  Market Trends
                </h2>
              </div>
              ...
              {market_trends?.developer_2023 &&
                market_trends.developer_2023.percentage && (
                  <div className="mb-4 p-4 bg-white border-1 rounded-md shadow-md border-pc">
                    <h3 className="text-xl mb-2 font-semibold text-gray-400">
                      <i className="fas fa-microchip ml-2"></i> React
                      Technologies Usage :
                    </h3>
                    <p>
                      <span className="text-pc font-medium">Percentage:</span>{" "}
                      {market_trends.developer_2023.percentage || "N/A"}
                    </p>
                    <p>
                      <span className="text-pc font-medium">Source:</span>{" "}
                      {market_trends.developer_2023.source || "N/A"}
                    </p>
                  </div>
                )}
